    ...
    internal class HostedHttpRouteCollection : HttpRouteCollection
    {
        ...
        public override IHttpRoute this[int index]
        {
            get
            {
                ...
                throw Error.ArgumentOutOfRange("index", index, "The index cannot be less than 0 or equal to or larger than the number of items in the collection.");
            }
        }
        ...
        public override IHttpRouteData GetRouteData(HttpRequestMessage request)
        {
            ...
            if (!request.Properties.TryGetValue(InternalConstants.HttpContextBaseKey, out httpContextBase))
            {
        ...
        {
            return Error.NotSupported("This operation is not supported by '{0}'.", typeof(HostedHttpRouteCollection).Name);
        }

        private static NotSupportedException NotSupportedByHostedRouteCollection()
        {
            return Error.NotSupported("This operation is only supported by directly calling it on '{0}'.", typeof(RouteCollection).Name);
        }
    }
}
